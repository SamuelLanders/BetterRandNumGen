...
# With reference to the PI normalized arrays below: storing these arrays in a separate file and loading as a global variable was roughly the same speed as placing the variables here, for singular runs of the function,
# however two things contrastly occurred: 1- repeated runs of the function get slightly (up to 6~20ms) faster when front loading the variable here vice placing the variable in the below function get_from_array(),
# 2- loading the arrays dynamically from a separate file from within the function KILLED run time performance (3x worse or more!).
# keeping the arrays defined here so that can attain the slight bump in performance for repeated runs of the random functions.
...
func generate_digits(v_need:int=1): #[v_need must be a number greater than 0 (not negative) ] #This can be useful for creating stuffed arrays of strings, representing random integers of varying lengths (eg. map seeds etc.).
	...
	for n in v_need:
		...
		if v_performStep==1: #shift up/right
			...
			v_piArrX=v_piArrX+(3*v_sliderRespArr[0])
			v_piArrY=v_piArrY-(3*v_sliderRespArr[0])
			...
			v_piArrX=v_piArrX+(3*v_sliderRespArr[0])	#Needs to be a significant jump so multiplying by an odd number (ensures can attain odd resultants)
			v_sliderArrX=v_sliderRespArr[1]
			v_sliderArrY=v_sliderRespArr[2]
			v_piResponseArray=get_from_array(v_piArrX,v_piArrY)
			v_randInt = v_randInt+v_piResponseArray[0]
